...
double Bernoulli::likelihood()
{  
    double qd = 1. - _p_detection + _p_detection * pow(_e_model->getBeta() / (_e_model->getBeta() + 1), _e_model->getAlpha());
...
double Bernoulli::likelihood(Cluster const& detection, Bernoulli*& bernoulli)
{
...
void Bernoulli::update_misdetection(Bernoulli*& bernoulli)
{
    ...
    double alpha = _e_model->getAlpha();
    double beta = _e_model->getBeta();
    ...
    bernoulli->_e_model->setAlpha(alpha_m);
    bernoulli->_e_model->setBeta(beta_m);
}

validation::ValidationModel* Bernoulli::getValidationModel()
{
    return _e_model->getValidationModel();
}